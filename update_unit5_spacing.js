...
async function run() {
  ...
  if (testData && testData.content_json) {
    ...
    if (readingPart) {
      ...
      html = html.replace(/<p style="margin-bottom: 1rem;">/g, '<p style="margin-bottom: 1.25rem; line-height: 1.6; color: #374151; font-size: 1.125rem;">');
      
      // Fix title case (remove uppercase)
      html = html.replace('text-transform: uppercase;', '');
      html = html.replace('THE JACKAL AND THE SUN CHILD', 'The Jackal and the Sun Child');
      ...
      if (error) {
        ...
        console.log('Successfully updated Unit 5 HTML spacing!');
      }
    }
  }
  process.exit(0);
