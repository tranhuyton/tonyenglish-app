...
async function uploadImages() {
  const folders = JSON.parse(fs.readFileSync('ielts_folders.json', 'utf-8'));
  const imagesDir = 'C:\\Users\\Tony\\.gemini\\antigravity\\brain\\a493a760-b548-4817-97c0-13b4ee51e9fa';
  
  const files = fs.readdirSync(imagesDir).filter(f => f.startsWith('ielts_cam_') && f.endsWith('.png'));
  
  for (const folder of folders) {
    const match = folder.title.match(/Cambridge (\d+)/i);
    if (!match) continue;
    
    const sectionNum = match[1];
    const imageFile = files.find(f => f.startsWith(`ielts_cam_${sectionNum}_`));
