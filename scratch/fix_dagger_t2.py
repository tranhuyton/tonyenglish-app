"""Fix &dagger; → &#273; (đ) encoding bug in all 6 Topic 2 pages."""
import requests
...
HDRS_GET  = {'apikey': KEY, 'Authorization': f'Bearer {KEY}'}
HDRS_PATCH = {'apikey': KEY, 'Authorization': f'Bearer {KEY}',
...
for name, pid in PAGE_IDS.items():
    r = requests.get(
        f'{SUPABASE_URL}/rest/v1/lecture_pages?id=eq.{pid}&select=id,content_html',
        headers=HDRS_GET
    )
    ...
    count_dagger = html.count('&dagger;')
    count_chi    = html.count('&#x2021;')   # double dagger just in case

    fixed = html.replace('&dagger;', '&#273;').replace('&#x2021;', '&#273;')

    if count_dagger + count_chi > 0:
        pr = requests.patch(
            f'{SUPABASE_URL}/rest/v1/lecture_pages?id=eq.{pid}',
            json={'content_html': fixed},
            headers=HDRS_PATCH
        )
        print(f'{name}: fixed {count_dagger} &dagger; + {count_chi} &#x2021; → HTTP {pr.status_code}')
    else:
        print(f'{name}: no &dagger; found — skipped')
