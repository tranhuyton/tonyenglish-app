                        ...
                        {(sec.questionType === "Trắc nghiệm" || sec.questionType === "TFNG") && (
                           <div className="space-y-6">
                             {(Array.isArray(sec.questions) ? sec.questions : []).map((q: any) => {
                                ...
                                if (isTFNG) {
                                    return (
                                     <div key={q.id} id={`q-${q.id}`} onClick={() => setActiveQuestionId(String(q.id))} className={`p-6 bg-white border rounded-none relative group transition-all flex flex-col md:flex-row gap-6 ${isReviewMode ? (isCorrect ? 'border-emerald-600 bg-emerald-50' : 'border-red-600 bg-red-50') : (activeQuestionId === String(q.id) ? 'border-black' : 'border-slate-400 hover:border-slate-600')}`}>
                                       <div className="flex items-start gap-4 font-sans flex-1 min-w-0 w-full md:w-auto">
                                         ...
                                         <div className="text-[16px] leading-relaxed font-bold text-black cursor-pointer flex-1 min-w-0 font-sans html-content-renderer [&>p]:!m-0" dangerouslySetInnerHTML={{ __html: cleanHtmlContent(q.content) }} />
                                       </div>
                                       
                                       <div className={`flex flex-row flex-wrap items-center gap-4 sm:gap-6 font-sans shrink-0`}>
                                         {validOptions.map((opt: any, i: number) => {
                                           ...
                                           return (
                                             <label key={i} className={labelClass}>
                                               ...
                                               <span className="text-[15px] font-bold text-black html-content-renderer font-sans [&>p]:!m-0" dangerouslySetInnerHTML={{ __html: cleanHtmlContent(safeOpt) }} />
                                             </label>
                                           );
                                         })}
                                       </div>
                                       ...
                                       {isReviewMode && (
                                          <div className="w-full mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-slate-300 md:pl-6 font-sans">
                                     ...
                                     {validOptions.map((opt: any, i: number) => {
                                       ...
                                       return (
                                         <label key={i} className={labelClass}>
                                           ...
                                           <span className="text-[15px] leading-[1.8] text-black font-sans html-content-renderer [&>p]:!m-0">
                                ...
                                return (Array.isArray(sec.questions) ? sec.questions : []).map((q: any) => {
                                    ...
                                    return (
                                     <div 
                                         ...
                                         className={`py-4 px-5 rounded-none border flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer transition-all ${isReviewMode ? (isCorrect ? 'bg-emerald-50 border-emerald-600' : 'bg-red-50 border-red-600') : (activeQuestionId === String(q.id) ? 'bg-slate-50 border-black' : 'bg-white border-transparent hover:border-slate-300')}`}
                                     >
                                       ...
                                       <div className="flex items-start gap-4 flex-1 min-w-0 w-full md:w-auto">
                                         ...
                                         <div 
                                             className="text-[16px] font-bold text-black leading-relaxed font-sans html-content-renderer flex-1 min-w-0 break-words [&>p]:!m-0" 
                                             dangerouslySetInnerHTML={{ __html: cleanHtmlContent(q.content) }} 
                                         />
                                       </div>
                                       
                                       {/* BÊN PHẢI: Box Kết Quả / Dropdown */}
                                       <div className="shrink-0 flex items-center justify-end w-full md:w-auto mt-2 md:mt-0 font-sans">
                                           {isReviewMode ? (
                                       ...
                                       {isReviewMode && (
                                          <div className="w-full mt-4 md:mt-0 pt-4 md:pt-0 border-t md:border-t-0 md:border-l border-slate-300 md:pl-6 flex-none basis-full md:basis-auto">
                                             <p className="text-[13px] font-black text-black uppercase mb-1">💡 Giải thích đáp án:</p>
                                             <div className="text-[15px] text-slate-800 font-medium leading-relaxed font-sans html-content-renderer" dangerouslySetInnerHTML={{ __html: cleanHtmlContent(q.explanation || 'Không có lời giải thích.') }} />
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex items-start gap-4 flex-1 min-w-0 w-full md:w-auto">
                                          ...
                                          <div className="text-[16px] font-bold text-black leading-relaxed font-sans html-content-renderer flex-1 min-w-0 break-words [&>p]:!m-0" dangerouslySetInnerHTML={{ __html: cleanHtmlContent(q.content) }} />
                                        </div>
                                ...
                                return combos.map((combo, comboIndex) => {
                                    ...
                                    if (isReviewMode) {
                                        if (isPerfect) {
                                            ...
                                            containerClass += " border-amber-600 bg-amber-50";
                                        } else {
                                            containerClass += " border-red-600 bg-red-50";
                                        }
                                    } else {
                                        if (comboIds.includes(activeQuestionId)) {
                                            containerClass += " border-black";
                                        } else {
                                            containerClass += " border-slate-400 hover:border-slate-600";
                                        }
                                    }
                                    ...
                                    return (
                                        <div key={`combo-${comboIndex}`} className={containerClass}>
                                          <div className="flex items-start gap-4 mb-5 flex-col w-full">
                                            ...
                                            <div className="text-[16px] leading-relaxed font-bold text-black cursor-pointer w-full flex-1 min-w-0 break-words font-sans html-content-renderer [&>p]:!m-0" dangerouslySetInnerHTML={{ __html: cleanHtmlContent(qText) }} />
                                          </div>
                                            ...
                                            {validOptions.map((opt: any, i: number) => {
                                              ...
                                              return (
                                                <label key={i} className={labelClass}>
                                                  ...
                                                  <span className="text-[15px] leading-[1.8] text-black font-sans html-content-renderer [&>p]:!m-0"><span className="font-bold mr-1 font-sans">{optionValue}.</span> <span dangerouslySetInnerHTML={{ __html: cleanHtmlContent(safeOpt) }} /></span>
                                                </label>
                                              );
                                            })}
                                          </div>
