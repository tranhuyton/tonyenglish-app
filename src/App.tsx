...
export default function App() {
  ...
  if (currentView === 'computer')
    return <ComputerTest onBack={handleBackToMenu} />;
  if (currentView === 'paper')
    return (
      <PaperTest
        onBack={handleBackToMenu}
        testData={selectedTest || localTestData}
      />
    );
      ...
      <div className="max-w-6xl w-full">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm mb-6 border border-gray-200">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xl uppercase ${
                user ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-400'
              }`}
            >
            ...
            <div>
              <p className="text-xs text-gray-500">
                {user ? 'Xin chào học viên,' : 'Khách truy cập'}
              </p>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm px-4 py-2 rounded-lg font-bold transition"
            >
              ⚙️ Quản Trị
            </button>
            {user ? (
              <button
                ...
                className="bg-red-50 hover:bg-red-100 text-red-600 text-sm px-4 py-2 rounded-lg font-bold transition border border-red-100"
              >
                Đăng xuất
              </button>
            ) : (
              <button
                ...
                className="bg-[#2b88c9] hover:bg-[#1a6ea6] text-white text-sm px-5 py-2 rounded-lg font-bold transition shadow-sm"
              >
                Đăng Nhập
              </button>
            )}
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden h-fit">
            <div className="bg-[#1f2937] px-8 py-6 text-center">
              <h1 className="text-2xl font-bold text-white mb-2">
                Thư Viện Đề Thi EdTech
              </h1>
              <p className="text-emerald-400 text-sm font-bold">
                Hệ thống đã đồng bộ Máy chủ & Tài khoản
              </p>
            </div>

            <div className="p-8">
              <h3 className="font-bold text-lg text-gray-800 mb-4 flex items-center gap-2">
                <span className="w-1.5 h-5 bg-purple-500 rounded"></span> Kho Đề
                Từ Đám Mây (Live Database)
              </h3>
              {isLoading ? (
                ...
                <p className="text-gray-500 italic text-sm py-4">
                  Chưa có đề thi nào trên Server.
                </p>
              ) : (
                <div className="grid sm:grid-cols-2 gap-6 mb-8">
                  {cloudTests.map((test) => (
                    <div
                      key={test.id}
                      className="border border-purple-200 bg-purple-50/30 rounded-xl p-5 hover:shadow-md transition group"
                    >
                      <span className="bg-purple-100 text-purple-700 text-[10px] font-bold px-2 py-1 rounded uppercase mb-2 inline-block">
                        LIVE CLOUD
                      </span>
                      <h2
                      ...
                      <button
                        onClick={() => handleStartTest('paper', test.test_data)}
              ...
              <h3 className="font-bold text-lg text-gray-800 mb-4 flex items-center gap-2">
                <span className="w-1.5 h-5 bg-blue-500 rounded"></span> Bài Thi
                Mẫu (Demo Offline)
              </h3>
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="border border-gray-200 rounded-xl p-5 hover:shadow-md transition">
                  <span className="bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-1 rounded uppercase mb-2 inline-block">
                    Thi Máy
                  </span>
                  ...
                  <button
                    ...
                    className="w-full bg-gray-800 hover:bg-black text-white text-sm font-bold py-2.5 rounded-lg transition"
                  >
                    Thi thử ➔
                  </button>
                </div>
                <div className="border border-gray-200 rounded-xl p-5 hover:shadow-md transition">
                  <span className="bg-emerald-100 text-emerald-700 text-[10px] font-bold px-2 py-1 rounded uppercase mb-2 inline-block">
                    Thi Giấy
                  </span>
                  <h2 className="text-base font-bold text-gray-800 mb-4">
                  ...
                  <button
                    ...
                    className="w-full bg-[#2b88c9] hover:bg-[#1f6696] text-white text-sm font-bold py-2.5 rounded-lg transition"
                  >
                    Thi thử ➔
                  </button>
                </div>
              ...
              {!user ? (
                <div className="text-center py-10">
                  ...
                  <p className="text-gray-500 text-sm">
                    Đăng nhập để xem lịch sử làm bài của bạn.
                  </p>
                  <button
                    onClick={() => setShowAuthModal(true)}
                    className="mt-4 text-sm font-bold text-[#2b88c9] underline"
                  >
                    Đăng nhập ngay
                  </button>
                </div>
                ...
                <div className="space-y-4">
                  {history.map((record, index) => (
                    <div
                      key={index}
                      className="bg-gray-50 border border-gray-200 p-4 rounded-xl relative overflow-hidden hover:shadow-md transition"
                    >
                      <div
                      ...
                      <h3
                        className="font-bold text-sm text-gray-800 truncate mb-1 pr-2"
                        title={record.test_title}
                      >
                        {record.test_title}
                      </h3>
