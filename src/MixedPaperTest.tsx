...
// TỰ ĐỘNG XÓA BỎ CÁC THUỘC TÍNH HEIGHT/OVERFLOW GÂY LỖI THANH CUỘN & BỌC BẢNG (TABLE WRAPPER)
const cleanHtmlContent = (html: any) => {
  ...
  // NÂNG CẤP FIX CHỒNG BẢNG: Bọc <table> bằng div để có thể scroll ngang mà không phải biến table thành "display: block"
  // 1. Tạm thời xóa wrapper cũ nếu bị gọi lồng nhau nhiều lần
  cleaned = cleaned.replace(/<div class="table-wrapper[^>]*>\s*(<table[^>]*>[\s\S]*?<\/table>)\s*<\/div>/gi, '$1');
  // 2. Bọc lại bằng wrapper chuẩn
  cleaned = cleaned.replace(/(<table[^>]*>[\s\S]*?<\/table>)/gi, '<div class="table-wrapper custom-scrollbar">$1</div>');
  
  return cleaned;
};

// --- COMPONENT CHÍNH ---
export default function MixedPaperTest({ onBack, testData, onFinish }: { onBack: () => void, testData?: any, onFinish?: (res: any) => void }) {
  ...
  const basicInfo = contentJSON.basicInfo || { title: safeTestData?.title || "IELTS Paper-based", timeLimit: "60", skill: "", category: safeTestData?.test_type === 'Mixed-Paper' ? 'exercise' : 'test' };
  ...
  if (!isReviewMode) {
    ...
    try {
      ...
      if (user) {
        ...
        // Auto-complete test-type assignments (Chỉ hoàn thành nếu đạt từ 50% điểm hoặc IELTS Band >= 4.0)
        const isPassed = (band && !isNaN(parseFloat(band)))
          ? parseFloat(band) >= 4.0
          : (total > 0 ? (score / total) >= 0.5 : score >= 5.0);
        if (isPassed) {
    ...
    <div className="flex flex-col h-screen bg-[#f3f4f6] font-serif text-gray-900 relative">
      ...
      <style>{`
          ...
          .format-passage table, .html-content-renderer table { 
              display: table !important; /* PHỤC HỒI BẢN CHẤT TABLE */
              width: 100% !important; 
              min-width: 600px !important; /* Bắt buộc bảng rộng tối thiểu để kích hoạt scroll ngang của wrapper */
              border-collapse: collapse !important; 
              margin: 0 !important; 
              max-width: none !important;
          }
          .format-passage thead, .html-content-renderer thead {
              display: table-header-group !important;
          }
          .format-passage tbody, .html-content-renderer tbody {
              display: table-row-group !important;
          }
          .format-passage tr, .html-content-renderer tr {
              display: table-row !important;
          }
          .format-passage th, .format-passage td,
          .html-content-renderer th, .html-content-renderer td { 
              display: table-cell !important; /* ĐẢM BẢO TH VÀ TD NẰM NGANG */
          ...
          .format-passage table p:last-child, .html-content-renderer table p:last-child {
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 relative bg-[#f3f4f6]" onMouseUp={handleMouseUp} ref={mainScrollRef as any}>
        ...
        <div className="flex mx-auto relative transition-all duration-300"
                          ...
                          <div key={sIndex} className="mb-10 font-sans">
                            ...
                            {shouldRenderGlobalSecContent && (
                               <div className="mb-6 text-[15px] text-slate-800 bg-gray-50 p-4 rounded-lg border border-gray-200 format-passage html-content-renderer font-serif">
                                 {sec.imageUrl && (
                                    <div className="mb-4">
                                       <img src={sec.imageUrl} alt="Section Illustration" className="mx-auto rounded border border-gray-200 max-h-[600px] object-contain shadow-sm" />
                                    </div>
                                 )}
                                                ...
                                                {validOptions.map((opt: any, i: number) => {
                                                  ...
                                                  return (
                                                    <label key={i} className={labelClass}>
                                                      <div className="relative inline-flex items-center justify-center w-[16px] h-[16px] shrink-0 mt-0.5">
                                                        ...
                                                        <div className={boxClass}>
                                                          {isSelected && (
                                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className={`w-4 h-4 absolute pointer-events-none ${isReviewMode ? (isCorrectOpt ? 'text-emerald-600' : 'text-red-600') : 'text-blue-600'}`}>
                            ...
                            {(isInlineDragDrop || isBlockDragDrop) && (
                              <div className="bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-sm">
                                ...
                                {isReviewMode && (
                                  <div className="mt-10 p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-inner font-sans">
                                    <p className="text-[13px] font-black text-gray-500 uppercase tracking-widest mb-4">Danh sách lựa chọn:</p>
                                    <div className="flex flex-wrap gap-3">
                                      {(() => {
                                        let allOptions: string[] = [];
                                        (sec.questions || []).forEach((q: any) => {
                                        ...
                                        return allOptions.map((opt: string, oIdx: number) => {
                                          ...
                                          return (
                                            <div
                                              key={oIdx}
                                              className="px-4 py-2 font-bold font-sans text-[14px] border border-gray-300 rounded bg-gray-50 text-slate-500 cursor-default select-none shadow-sm"
                                            >
                                              {displayOpt}
                                            </div>
                                          );
          ...
          {/* THANH KÉO DỌC (SPLIT PANE) CHO LISTENING TRONG TRƯỜNG HỢP GẶP CÂU HỎI QUÁ RỘNG */}
          {isListening && !isReviewMode && !hasAnySplitPane && (
            <div
