...
export default function AdminPanel({ onNavigate }: { onNavigate?: (view: string) => void }) {
  ...
  const handleInitiateTest = (mode: 'manual' | 'import') => {
    setShowCreateDropdown(false);
    setEditingTest({ id: 'new', title: '', folder_id: '', test_type: 'IELTS-Listening', content_json: null, mode });
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        ...
        {/* ========================================================= */}
        {/* HEADER CHÍNH - ĐÃ GỘP LUÔN CẢ NÚT TẠO ĐỀ VÀO ĐÂY ĐỂ TRÁNH LỖI Z-INDEX */}
        {/* ========================================================= */}
          ...
          <div className="flex items-center gap-4">
            ...
            {activeTab === 'library' && (
              <div className="relative shrink-0" ref={dropdownRef}>
                ...
                {showCreateDropdown && (
                  <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 py-2 animate-in fade-in zoom-in-95">
                    <button onClick={() => handleInitiateTest('manual')} className="w-full text-left px-5 py-3 hover:bg-slate-50 font-bold text-[13px] text-slate-700 border-b border-slate-100">✍️ Tạo thủ công</button>
        ...
        <div className="flex-1 p-8 overflow-y-auto">
          ...
          {/* TAB: COURSE DETAIL (DRILL-DOWN FOLDERS NHƯ HỌC SINH) */}
          {activeTab === 'course-detail' && selectedCourse && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
                ...
                <div className="p-8">
                  {/* Nếu có thư mục con -> Hiện Grid Ô vuông */}
                  ...
                  {/* NẾU LÀ THƯ MỤC LÁ (KHÔNG CÓ CON) -> MỚI CHO GÁN ĐỀ THI */}
                           ...
                           {currentTests.map(t => (
                             <div key={t.id} className="bg-slate-50 border border-slate-200 p-4 rounded-xl flex justify-between items-center group hover:bg-white transition-colors">
                               <div className="flex items-center gap-3">
                                 <span className="text-lg">{t.test_type.includes('Listening') ? '🎧' : '📖'}</span>
                  ...
                  {/* ROOT CỦA KHÓA HỌC MÀ CHƯA CÓ FOLDER */}
                  {!currentFolderId && currentSubFolders.length === 0 && (
          ...
          {activeTab === 'library' && (
            <div className="space-y-6">
              ...
              {/* TÌM KIẾM, LỌC VÀ THAO TÁC HÀNG LOẠT */}
