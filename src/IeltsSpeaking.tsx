...
export default function IeltsSpeaking({ onBack, testData: propTestData, onFinish }: { onBack?: () => void, testData?: any, onFinish?: (res: any) => void }) {
  ...
  // 🚀 HÀM GỌI ĐIỆN VÀ THU ÂM SONG SONG
  const startLiveCall = async () => {
    ...
    try {
      ...
      // 1. CHẠY THU ÂM NGẦM ĐỂ NỘP BÀI CHẤM ĐIỂM SAU
      ...
      timerRef.current = setInterval(() => {
        setRecordingTime(prev => {
           if (prev >= currentQ.maxTime - 1) {
              stopLiveCall(); // Hết giờ tự động cúp máy
      ...
      // 2. MỞ ỐNG NƯỚC LIVE SPEAKING VỚI GEMINI
      ...
      ws.onopen = async () => {
        ...
        // MỚM LỆNH CHO AI HỎI ĐÚNG CÂU HỎI HIỆN TẠI
      ...
      ws.onmessage = async (event) => {
        try {
          ...
          if (msg.setupComplete) {
             ...
             // LỆNH MỒI: YÊU CẦU AI ĐỌC CÂU HỎI TRƯỚC
  ...
  const handleReviewToggle = () => setReviewFlags(prev => ({ ...prev, [currentQIndex]: !prev[currentQIndex] }));
  ...
  // 🚀 LOGIC NỘP BÀI CHẤM ĐIỂM ĐƯỢC GIỮ NGUYÊN (VẪN GỬI AUDIO LÊN AI GRADER)
  const handleFinalSubmit = async () => {
    ...
    if (!window.confirm(`Bạn đã hoàn thành ${answeredQuestions}/${totalQuestions} câu. Bạn đã sẵn sàng nộp bài cho hệ thống chấm điểm?`)) return;
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('ai-grader', {
        body: { prompt: prompt, base64Audio: base64Audio, model: 'gemini-2.5-flash' }
      });
            ...
            <section className="w-full md:w-1/2 bg-[#f4f4f4] flex flex-col overflow-hidden relative">
              ...
              <div className="p-8 md:p-10 shrink-0">
                  <div className="h-[90px] bg-white border border-slate-400 rounded-none flex items-center justify-between px-6">
                      ...
                      {/* NẾU ĐÃ GỌI XONG THÌ HIỆN TRÌNH PHÁT LẠI */}
                      {audioUrls[currentQ.id] ? (
                         ...
                         <div className="flex items-center gap-6 w-full">
                            ...
                            {/* ĐỒNG HỒ ĐẾM NGƯỢC THỜI GIAN GỌI */}
             ...
             <div className="flex items-center h-full shrink-0">
                ...
                <div className="flex items-center gap-3 h-full pl-6">
                   {allQuestions.map((q, idx) => {
                      ...
                      else if (isAnswered) bgClass = 'bg-emerald-100 text-emerald-800 border-emerald-500 font-black';
